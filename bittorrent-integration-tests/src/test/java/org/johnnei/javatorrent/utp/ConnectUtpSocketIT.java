...
public class ConnectUtpSocketIT {
    ...
    public void testConnectThenDisconnect() throws Exception {
        ...
        assertEquals(
            "Connection state on local socket must have transitioned to connected.",
            ConnectionState.CONNECTED,
            localSocketImpl.getConnectionState()
        );

        Map<Short, UtpSocketRegistration> localSockets = Whitebox.getInternalState(Whitebox.getInternalState(
            localUtpModule,
            UtpMultiplexer.class
        ), "utpSockets");
        ...
        assertEquals(
            "Remote socket must still be in the connecting state",
            ConnectionState.CONNECTING,
            remoteSocketImpl.getConnectionState()
        );
        ...
        assertEquals(
            "Connection state must have transitioned on local socket",
